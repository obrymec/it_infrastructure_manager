...
function MessageBox (parent, data = new Object ({}), auto_hide = false, id = null) {
	...
	this.visibility = (visible, finished = null) => {
		...
		if (visible) $ (msg_id).css ("display", "flex").animate ({opacity: 1}, 300, () => {
			// Checks whether the passed value is a function.
			if (typeof finished === "function") finished (); apply_css_animation (new Object ({name: "message_box",
			duration: 200, ref: __ (ctr_id)}), new Object ({transform: "translate(0, 0)"}));
		// Hides the message box.
		}); else $ (msg_id).animate ({opacity: 0}, 300, () => {
			// Checks whether the passed value is a function.
