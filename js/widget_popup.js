...
* @fileoverview Widget popup component.
* @file widget_popup.js
* @created 2021-12-17
* @updated 2024-04-21
* @supported DESKTOP
* @version 0.0.2
...
function WidgetPopup (parent, data = new Object ({}), id = null) {
	...
	this._build = () => {
		...
		if (parent != null) {
			...
			// Fixing resize event.
			this.override_resizable ((data.hasOwnProperty ("max_width") ? data.max_width : null),
				(data.hasOwnProperty ("max_height") ? data.max_height : null));
	...
	this.visibility = (visible, finished = null) => {
		...
		if (!visible) {
			...
			$ (ctr_id).css ("transform", "scale(1.3)").css ("opacity", 0); $ (wig_id).animate ({opacity: 0}, 300, function () {
		...
		// Otherwise.
		} else $ (wig_id).css ("display", "flex").animate ({opacity: 1}, 300, () => {
			// Shows the widget popup container.
			$ (ctr_id).css ("transform", "scale(1)").css ("opacity", 1); if (typeof finished === "function") finished ();
		});
	}
