	...
	if (network_manager () && !window.lock) {
		...
		// Checking the given data.
			...
			if (Array.isArray (server.errors)) message_box (server.errors [0].message); else {
				...
				set_cookie ("it_user", server.user_id, (20 / 60));
				load_sign ("./sources/features/dashboard/dashboard.html");
			}
...
function sign_up () {
	...
	if (network_manager () && !window.lock) {
		...
		// Checking the given data.
			...
			// The server data contains some errors.
			} if (Array.isArray (server.errors)) message_box (server.errors [0].message);
			else load_sign ("./sources/features/sign/sign.html");
		// Disables any potentials actions.
		}, null, 180000, 0, true); window.lock = true;
	}
}
...
$ (() => {
	...
	$ ("div.img-zone > img").attr ("src", (window.SIGN_IN ?
	"./public/assets/images/sign_in.jpg" : "./public/assets/images/sign_up.jpeg")).on ("load", () => {
		...
		window.setTimeout (() => {
			...
			sign_animation (1, () => {$ ("input").css ("pointer-events", "auto"); let options = $ (sign_options).children ();
				...
				options [0].addEventListener ("click", () => load_sign ("./sources/features/sign/sign.html"));
