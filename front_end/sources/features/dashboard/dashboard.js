...
function add_equipment () {
	...
	if (network_manager ()) {
		...
		let today = get_date ();
		draw_basic_widget ("./sources/features/dashboard/add_equipment.html", 600, "Registering equipment", "eq-add", () => {
...
function show_history (card) {
	...
	if (card instanceof DataCard && network_manager ()) {
		...
		draw_basic_widget ("./sources/features/load_history/load_history.html", 600, ("History of " + title), "eq-hty", (widget) => {
...
function generic_task (type, title, ready = null, drop_init = null, card_ref = null, toolbar = null) {
	...
	if (network_manager ()) {
		...
		let today = get_date (); draw_basic_widget ("./sources/utils/generic_task.html", 280, title, "gen-tsk", (widget) => {
			// Initializes dates value.
...
$ (() => {
	...
	// Fixing "click" event on "equipments" option.
		...
		select_option ("div.equipments", "./sources/features/load_equipments/load_equipments.html", props_to_destroyed);
	// Fixing "click" event on "services" option.
		...
		select_option ("div.services", "./sources/features/load_services/load_services.html", props_to_destroyed);
	// Fixing "click" event on "users" option.
		...
		select_option ("div.users", "./sources/features/load_users/load_users.html", props_to_destroyed);
	// Fixing "click" event on "bugs" option.
		...
		select_option ("div.problems", "./sources/features/load_problems/load_problems.html", props_to_destroyed);
	// Fixing "click" event on "providers" option.
		...
		select_option ("div.providers", "./sources/features/load_providers/load_providers.html", props_to_destroyed);
	// Fixing "click" event on "disconnect" option.
		...
		if (network_manager () && window.DISCONNECT) {
			...
			window.SIGN_IN = true; $ ("div.drp").css ("top", "-100%"); dashboard_visibility (false, () => {
				...
				load_view ("./sources/features/sign/sign.html", "div.views", "Loading...", "Loading login page...");
			});
		}
	// Animates the dashboard.
	}); dashboard_visibility (true, () => {
		// Gets active option tag name attribute value.
		const optName = document.querySelector(window.active_option).getAttribute("name");
		// Loads the default dashboard option.
		select_option (window.active_option, `./sources/features/${optName}/${optName}.html`, [], true);
	// Removes this script.
	}); $ ("script").remove ();
});
